// Package Firefox extension into zip artifact.
...
async function main() {
  ...
  // Create zip file
  console.log(`Packaging Firefox extension: ${zipName}`);
  execSync(`cd "${DIST}" && zip -r "${zipPath}" .`, { stdio: 'inherit' });

  console.log(`Firefox extension packaged: ${zipPath}`);
  ...
  const listCommand = `unzip -l "${zipPath}" | grep -E '(\.git|node_modules|tests|docs|\\.map|artifacts|dist/chrome|\\.env)' || echo 'No unwanted files found'`;;
